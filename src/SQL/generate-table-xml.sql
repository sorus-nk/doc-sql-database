...
from (
	select 
		...
		+ case when ep.value is not null then '" description="' + replace(cast(ep.value as varchar(255)), '"', '&quot;') else '' end
		+ '">' as [tableXml],
		'0.1' as [sort_order]
	from sys.databases db
	left join sys.extended_properties ep on ep.major_id = db.database_id and ep.class_desc = 'DATABASE' and ep.name = 'MS_Description'
	where 
	...
	left join sys.extended_properties ep on ep.major_id = t.object_id and ep.minor_id = 0
		and ep.class_desc = 'OBJECT_OR_COLUMN' and ep.name = 'MS_Description'
	...
	left join sys.extended_properties ep on ep.major_id = v.object_id and ep.minor_id = 0
		and ep.class_desc = 'OBJECT_OR_COLUMN' and ep.name = 'MS_Description'
	...
	inner join sys.types ty on c.user_type_id = ty.user_type_id 
	left join sys.extended_properties ep on ep.major_id = v.object_id and ep.minor_id = c.column_id
		and ep.class_desc = 'OBJECT_OR_COLUMN' and ep.name = 'MS_Description'
	--where v.type = 'U'
